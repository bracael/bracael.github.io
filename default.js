...
starCountRef.once('value', function(snapshot) {
	...
	document.head.insertAdjacentHTML('beforeend', `<script src="/feeds/posts/default/${dataProduct}?alt=json-in-script&callback=HTTPXMLresquest" type="text/javascript"></script>`);

document.querySelectorAll('[data-product]')[i].addEventListener('click', function(event){

	document.getElementById('HTTPXMLresquest').src = `/feeds/posts/default/${this.getAttribute('data-product')}?alt=json-in-script&callback=HTTPXMLresquest`;
